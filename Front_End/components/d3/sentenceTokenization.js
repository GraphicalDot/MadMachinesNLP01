...
App.BodyView = Backbone.View.extend({
	...
	initialize: function(){
		...
		$(".submitText").click(function(){
			...
			var jqhr = $.post("http://52.74.143.163:8080/sentence_tokenization", {"text": text, "link": null})
		...
		$('.modal-trigger').leanModal({
				...
				complete: function() {
					...
					var jqhr = $.post("http://52.74.143.163:8080/sentence_tokenization", {"text": null, "link": __value})
...
App.SentencesView = Backbone.View.extend({
		...
		sendButton: function(event){
			...
			var jqhr = $.post("http://52.74.143.163:8080/upload_sentence", {"sentence": this.model.sentence, "sentiment": self.$(".polarity").val(), "tag": self.$(".tags").val(), "subtag": self.$(".subTags").val()} )
			jqhr.done(function(data){
				console.log(data)
